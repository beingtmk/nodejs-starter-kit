...
class AdminGroups extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (!this.props.groupLoading) {
      const endCursor = this.props.groups ? this.props.groups.pageInfo.endCursor : 0;
      const prevEndCursor = prevProps.groups ? prevProps.groups.pageInfo.endCursor : null;
  ...
  render() {
    return <AdminGroupsView {...this.props} />;
  }
}
...
const onAddGroup = (prev, node) => {
  ...
  const filteredPosts = prev.groups.edges.filter(post => post.node.id !== null);
