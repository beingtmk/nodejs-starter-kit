...
class AddPublicQuizToGroupComponent extends React.Component {
  ...
  handleAddQuizToGroup = async (e) => {
    const { addQuizToGroup, childGroup } = this.props;
    console.log(e);
    const input = {
  ...
  render() {
    console.log("QuizImportFromParentGroupComponent", this.props);
    const { groupQuizzesLoading, childGroupQuizzes, groupQuizzes } = this.props;
    const { selectedRows } = this.state;
    ...
    var notInGroup =
      groupQuizzes && groupQuizzes.filter(comparer(childGroupQuizzes));
